---
# DVTD-ecnx
title: 'Start from gate X: make the checkpoint a visible choice'
status: todo
type: feature
priority: high
...
**What:** Offer a planted checkpoint as a named press at run start, instead of spending it silently.

**Why:** Starting a run burns the checkpoint as a side effect, so a player cannot reason about something they paid for.

## Done when
- [ ] Decided: whether a plain start still burns the checkpoint, and the decision doc says so
- [ ] With a checkpoint planted, the entry screen offers "Start from gate X" beside a plain start
- [ ] The press names what it grants: the gate, its name, the slots and the storage
- [ ] A plain start never consumes it silently
- [ ] A checkpoint pointing at a gate that no longer exists cannot start a broken run
- [ ] The same press appears where a run ends
