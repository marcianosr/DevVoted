...
updated_at: 2026-08-31T10:26:37Z
---
...
- [x] npm run db:push on the local database (verified applied 2026-08-31: users.owned_swatch_ids present locally; full schema.ts vs local DB diff clean across 12 tables / 107 columns)
...
Closed 2026-08-31. The last open box was stale, not outstanding work.

The schema half shipped on both required paths per ADR-012: `users.owned_swatch_ids` is declared in `src/database/schema.ts` and ships as the guarded migration `supabase/migrations/20260806120000_add_owned_swatch_ids.sql`, which CI applies to production on merge.

The local application was verified directly rather than assumed. A read-only `information_schema.columns` query against the local Supabase (localhost:54322) confirms `users.owned_swatch_ids` is present, and a full diff of every table and column declared in `schema.ts` against the live local database reports no drift: 12 tables, 107 columns, clean. Nothing left to push.

Verification of the surrounding tree at close: `npm run lint` clean (786 modules, 3219 dependencies, 0 violations), `npm run build` + `tsc --noEmit` clean, `npm test` 2616 passed / 6 skipped / 2 todo.
