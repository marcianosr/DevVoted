---
# DVTD-svbl
title: 'Config: S3 Glacier banks more when a run dies'
status: draft
type: feature
priority: normal
tags:
...
**What:** A config that makes a dying run bank a bigger share of its storage.

**Why:** The only config you would buy while losing, which nothing else in the roster does.

## Done when
- [ ] The size and shape of the improvement are decided
- [ ] A death with it installed banks more than one without
- [ ] Decided: whether it must survive the final peel to pay, and what a rescued run gets
- [ ] A spec per case
