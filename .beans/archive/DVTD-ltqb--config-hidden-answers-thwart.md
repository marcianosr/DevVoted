...
updated_at: 2026-09-16T18:56:30Z
parent: DVTD-72d9
---
...
Closed 2026-09-16 for the Phase 1 it delivered. Every Shipped bullet above was
verified against the code: the 451 audit (`audit.model.ts:106`, `REDACTED_POLL_COUNT`,
`REDACTED_PER_POLL`, `BUY_BACK_KB`), the `poll-reading` family blocking 300 and
404, pools A and C, `redactedOptionIdsFor` pure and blind to `correct` with a
floor of 2 (`audit.model.ts:386`), `buyBackFeeFor`/`canBuyBack`/`spendBuyBack`,
`RunState.boughtBackOptionIds` never resetting, server-side redaction in
`redactPoll`, and the unseal press on the answer row in both themes. The linter
skips sealed answers (`paidAction.model.ts:28`).

**One Shipped bullet is wrong:** it claims explicit `DENY_PAIRS` against 403.
There is no such entry — `auditSchedule.model.ts:38` holds only
`[["mirrored", "timeout"]]`. That is deliberate, not a miss: ADR-058 Decision 2
records that the pair was added and then removed once Decision 4 exempted the
buy-back from 403's freeze.

**Phase 2 is now DVTD-t836**, under the epic DVTD-72d9, carrying the social half
as seven real todos: the filing intent record, the shop press, the
32/64/128/256 KB ladder and 3x cap, the service-layer toll, filer attribution,
filer immunity, and the guard against a filed 451 co-drawing with 403. It also
replaces the dangling **DVTD-w5pb** reference in this bean and in ADR-058, which
points at a bean that does not exist.
