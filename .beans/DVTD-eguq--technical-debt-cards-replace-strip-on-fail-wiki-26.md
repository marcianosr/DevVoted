...
This replaces the current strip-on-fail flow (`closeWindow` → "awaiting-strip" → `strip()`/`resumeClimb` in `src/modules/run/run/run.model.ts`) and answers DVTD-civm's open death-model question ("fully debted" as the terminal state).
