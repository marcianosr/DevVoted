...
const Row = ({ row, first }: { row: LedgerRow; first: boolean }) => {
...
export const LedgerRows = ({ rows, meter }: LedgerRowsProps) => (
	<div className={ROWS}>
		{rows.map((row, index) => (
			<Row key={index} row={row} first={index === 0} />
		))}
		{meter === undefined ? null : (
			<div className={METER_ROW}>
				<Meter {...meter} />
			</div>
		)}
	</div>
