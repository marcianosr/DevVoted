...
// A grid, so the rule below always starts under the name rather than under the
// icon, on every row, whatever the icon is.
const AUDIT_ROW =
...
/** Matches Row's own word for the same effect, so a dimmed audit and a dimmed
 * pipeline row fade by the same amount. */
...
/** The redaction is in the type, not in the rendering: an unseen audit has no
 * name to hand over, so no caller can leak one into the markup and rely on the
 * panel to hide it. Same shape SwatchTrackItem uses for the colour you have not
 * earned yet. */
