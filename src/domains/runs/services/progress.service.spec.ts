...
describe("incrementRunProgress", () => {
	...
	describe("storage drain", () => {
		...
		it("drains storage on wrong answer when drain slot is active", async () => {
			const mockRun = createMockRun({
				pipelineSlots: [drainSlot],
				categoryCoverage: [
					createMockRunCategoryCoverage({
						categoryCode: "js",
						currentCoverage: 10,
			...
			vi.mocked(awardCoverageToRun).mockResolvedValue(
				createMockRunCategoryCoverage({ categoryCode: "js" })
			);
			vi.mocked(drainStorage).mockResolvedValue(undefined);

			const result = await incrementRunProgress({
				categoryCode: "js",
				run: mockRun,
				correctnessFactor: 0,
				...createTestPollContext("js"),
			});

			const expectedDrain = 10 * 1024; // normal tier: 10kb per wrong answer
			expect(drainStorage).toHaveBeenCalledWith(mockRun.id, expectedDrain);
			expect(result.storageDrained).toBe(expectedDrain);
		});

		it("does not drain storage on correct answer", async () => {
			const mockRun = createMockRun({
				pipelineSlots: [drainSlot],
				categoryCoverage: [
					createMockRunCategoryCoverage({
			...
			expect(drainStorage).not.toHaveBeenCalled();
			expect(result.storageDrained).toBe(0);
		});

		it("does not drain storage when no drain slot is in the pipeline", async () => {
			const mockRun = createTestRun(); // no pipeline slots (empty by default)
		...
		it("returns storageDrained 0 when answer is correct", async () => {
