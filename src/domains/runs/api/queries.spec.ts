...
describe("Run Queries", () => {
	...
	describe("getRandomExposedConfigDeck", () => {
		it("returns null when no active runs exist", async () => {
			...
			const result = await getRandomExposedConfigDeck(
				"sheldon-cooper-id",
				"2024-12-25"
			);
		...
		it("returns a random deck when active runs exist", async () => {
			const mockActiveRuns = [
				{
					userId: "leonard-hofstadter-id",
					displayName: "Leonard Hofstadter",
					photoUrl: "https://example.com/leonard.jpg",
					configIds: [".js-config", "eslint-config"],
					runId: 42,
				},
				{
					userId: "howard-wolowitz-id",
			...
			const whereMock = vi.fn().mockResolvedValue(mockActiveRuns);
			const innerJoinMock = vi.fn().mockReturnValue({ where: whereMock });
			const fromMock = vi.fn().mockReturnValue({ innerJoin: innerJoinMock });
			...
			expect(result?.userId).toBeOneOf([
				"leonard-hofstadter-id",
				"howard-wolowitz-id",
			]);
			expect(result?.displayName).toBeOneOf([
				"Leonard Hofstadter",
				"Howard Wolowitz",
			]);
			expect(result?.runId).toBeOneOf([42, 43]);
		});

		it("returns same deck for same date (seeded randomness)", async () => {
			const mockActiveRuns = [
				{
					userId: "rajesh-koothrapalli-id",
					displayName: "Rajesh Koothrapalli",
					photoUrl: null,
					configIds: [".py-config"],
					runId: 44,
				},
				{
					userId: "penny-id",
					displayName: "Penny",
					photoUrl: "https://example.com/penny.jpg",
					configIds: [],
					runId: 45,
				},
			];
			...
			// Same date should return same result
			const result1 = await getRandomExposedConfigDeck(
				"sheldon-cooper-id",
				"2024-05-13"
			);
			const result2 = await getRandomExposedConfigDeck(
				"sheldon-cooper-id",
				"2024-05-13"
			);
		...
		it("returns different deck for different dates", async () => {
			const mockActiveRuns = [
				{
					userId: "banjo-id",
					displayName: "Banjo",
			...
			// Different dates may return different results (with enough variety in data)
			const result1 = await getRandomExposedConfigDeck(
				"gruntilda-id",
				"2024-12-25"
			);
			...
			// With 4 options and different seeds, there's a good chance they differ
			// But we can't guarantee it, so just check both are valid
			expect(result1).not.toBeNull();
			expect(result2).not.toBeNull();
		});

		it("returns deck with correct structure", async () => {
			const mockActiveRuns = [
				{
					userId: "tooty-id",
					displayName: "Tooty",
					photoUrl: "https://example.com/tooty.png",
					configIds: ["public-config", ".html-config", "intellisense-config"],
					runId: 99,
				},
			];
