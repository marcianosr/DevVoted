...
- **The shop speaks in rows and buttons**: the shop's offers list like pipeline rows — a ＋ mark, the config's chip, its demand/payoff story, and a green **buy 32KB** button on the right; an offer you can't afford dims and reads "need 128KB" instead. Your load-out rows carry their verbs the same way: an **upgrade** button priced in coverage — shown only when the config can actually level — and **sell** with its refund; no more hidden sell/upgrade popover behind the chip. With a full pipeline the buy buttons park and their tooltip says how to make room; "Rebuild configs" is now "Reroll offers" (same fee), and the shop's continue button names its destination: "Continue to gate 2 →".
