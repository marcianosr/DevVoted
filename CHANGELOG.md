...
## Unreleased
- **Gates count from 0**: a run now opens on **gate 0** and summits on **gate 11** — you start at zero and work toward one, the way a counter should read. Every gate label moved with it, so your first day is "gate 0" and a first-gate break reads "Gate 0 — pipeline broke here". The swatch that opens each gate shifted to match: Pallet opens gate 0, Boulder gate 1, and the Elite Four gate 11.
- **Storage reads as free space**: the HUD leads with what you have left to spend — a big "328 KB free" over a bar of what's committed, with "184 of 512 used" underneath — because headroom is what you budget against in the shop, and anything earned past the 512KB cap is discarded. (The old help text still claimed a 1 MB cap; it doesn't.)
- **Every gate pip tells its own story**: the climb's bar is now a row of chunky pips, one per gate, each in the colour of the swatch that opens it. Hover or tap a pip and it names that swatch, the coverage it costs, what you hold, and the gap left — "Thunder Swatch · gate 3 · Opens at 28% coverage · you have 19% · 9% to go".
- **The climb reads as one bar**: the HUD's "Gate 1 / 12" becomes "gate 3 · 2 / 12 cleared" over a twelve-segment bar that *is* the swatch ladder — each segment wears the colour of the swatch that opens its gate, so the collection you're working toward is visible at a glance. Gates you hold read solid, the rung you're currently buying fills with the coverage you've earned toward it, and the ones still ahead sit dimmed in their own colours (the Elite Four's shimmers). Since coverage is what buys each advance, the two numbers that used to sit apart now tell one story. Hover the bar and the whole ladder opens: every gate, the swatch that opens it — each in its own colour — and the coverage it costs, turning green once you can afford it, so you can see what the rest of the run will ask for without leaving the poll you're on. On the run setup screen the reward and multiplier stats are labelled **Gate modifiers**, and your swatch collection moved out from under that heading into its own line where it belongs.
- **Collect the badges: every slot you unlock is a swatch**: the eleven pipeline slots beyond your starting three each carry one — the eight gen-1 gym badges (Boulder, Cascade, Thunder, Rainbow, Soul, Marsh, Volcano, Earth), then Lavender and Seafoam for the two Kanto spots that never had a gym, and the Elite Four to finish at slot 14. Every journey starts in Pallet: the third of your starting slots carries the **Pallet Swatch**, free from the moment you begin, so your collection is never empty and gate 1 has an opener like every other gate. Each wears its home town's color; the Elite Four wears the legendary gradient. The shop shows the next one as a full-width row: its swatch, the gate it opens, the coverage it wants against what you have, a live progress bar, and an **Unlock slot** button the moment you're over the line; the old dashed "Add slot" tile is gone. Swatches are yours forever, across every run — see the ones you hold on the gate-cleared screen, your run setup stats, the end-of-run summary, and a new **Swatches** tab in the Dex where the ones you haven't earned stay hidden behind "???".
- **The summit moves to gate 12, and every gate past the first is bought with a slot**: gate 1 runs on your three starting slots, gate 2 needs slot 4, and gate 12 needs all fourteen. Clear the deepest gate your pipeline can hold and you'll replay it — the clear still pays in full, but the climb waits until you widen, and **unlocking the slot is the moment the gate advances**. A replayed gate keeps its demands and payout exactly as they were, so there's nothing to farm. The gate-cleared screen says so plainly when it happens: same green PASS, but the headline reads "Gate 3 cleared — still gate 3" and points at the swatch that would carry you on. One knock-on: because dying banks a share of your leftover storage proportional to how far you climbed, and the summit is now twelve gates instead of five, breaking at gate 5 banks 42% instead of everything.
- **Your build is the whole checklist**: gates no longer carry a built-in correct-answer demand — only your configs' checks judge you, with Unit Tests owning the escalating correct-count. In exchange, a cleared gate pays by how deep you are and how well you answered: 32 KB × the gate number, scaled by your correct answers — gate 1 tops out at 32 KB, gate 5 at 160, growth stops at gate 12, and a 0-correct clear pays nothing. A pipeline stripped bare can no longer clear a gate at all.
- **The wait wears a clock**: when your run is locked for the day, the community board's continue button counts down to tomorrow's polls ("New polls in 7h 23m") and turns back into "Climb on →" once the new day starts.
- **Answer reviews wear their category**: each row of "Review your answers" opens with a small swatch in the poll's category color, and the choices fold out exactly under the question in smaller type.
...
- **The main action wears the screen's color**: the primary button (submit answer, next, continue) follows the category theme — saffron on a JavaScript poll, cerulean on CSS — and keeps celadon green on screens without a category (start run, the shop). Config chip labels read white everywhere, leaving the border and tint to carry the rarity color.
...
- **Community board** (`/run/community`): after the shop, the climb stops by the Community page ("Community →"). Every poll of the day lists each answer option as one line — the option (the right answer reads green), the avatar chips of exactly who picked it (you first) and the pick count on the right; hover or tap a chip for the name. On phones each poll folds to its question plus "X% had it correct" — tap to open; desktop shows everything. Skipped polls stay sealed. Under the polls, **standouts today** crowns the day's champions: fastest answer (answers are now timed from the moment a poll appears to your submit), first to answer after the day's polls dropped, and most polls of one category.
