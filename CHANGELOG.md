...
## Unreleased
- **One color for the main action**: the primary button (start run, submit answer, next, continue) is now always celadon green — the same color as the linter's "use" button and the preview's "click to add" — instead of taking the screen's category color. Config chips flipped the same way: their labels now read white everywhere, leaving the border and tint to carry the rarity color.
...
- **The pipeline is a table now**: every pipeline surface (run setup, answering, gate report, shop load-out) lines its configs up under four column headers — **config**, **effect**, **condition**, **state**. Each row leads with a state mark (an amber dot while its check runs, a green ✓ passed, a red ✗ failed, gray dormant, a hollow ring for checkless configs — which read "passive", and a ▸ on a usable-but-idle linter), then the config's name with its rarity in that rarity's color, its effect in green ("×1.5 Ruby", "+8KB each, to 320KB"), its condition ("1 correct Ruby", "3 correct" — a linter's escalating fee reads red), and its live counter on the right. The copy got terse to fit the grid; the full story stays in each config's tooltip. On narrow screens the headers step aside and each row folds to two lines — name and counter up top, effect and condition beneath — with the columns still lining up. Code Coverage's row no longer prints "steady" while your streak is clean — no news is good news. The linter acts from its own row: its "use 8KB" button replaces the poll card's old "Run linter" button. The answering screen's pipeline section is headed "Your pipeline" with the slots counted underneath ("3 of 4 slots used") — the same heading the run setup screen uses; the poll's category header is smaller, and the colored line under it is gone. The shop's chips stay as the click-to-sell/upgrade target in their row; only the answer review keeps its boxed badges.
