...
## Unreleased
- **Pipeline rows read like a build log**: on the run setup and answering screens each pipeline config is a row with a state dot (amber while its check runs, green passed, red failed, gray dormant, a hollow ring for checkless configs — which now read "passive"), its name, and labeled lines for what it **gives** (green), **needs**, and **costs** (red). The linter acts from its own pipeline row: a teal ▸ marks it usable and a "use 8KB" button replaces the poll card's old "Run linter" button. The answering screen's pipeline header counts your slots ("pipeline · 3 of 4 slots"), the poll's category header is smaller, and the colored line under it is gone. The gate report and the shop's load-out use the same rows (the shop's chips stay as the click-to-sell/upgrade target); only the answer review keeps its boxed badges.
