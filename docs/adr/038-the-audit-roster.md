...
Accepted 2026-08-17 (Marciano, DVTD-60he, extended the same day by DVTD-lhao).
Fills out [ADR-035](035-gates-are-auditors.md) Decision 4, which shipped four
audits and left gates 3–6, 8 and 10 as open content slots.
...
| Read-only | `closesShop` | The shop *before* this gate refuses every write. Dropping still works: it belongs to the gate door, not the till. |
...
| Rolling Outage | `disablesConfig: "rotating-per-poll"` | Steps through the build, a different config down each poll. |
...
the picks the window will ask for.

A poll whose options are all correct is left unmirrored: there would be nothing
to pick, and an unanswerable poll is a soft-lock rather than a debuff.
...
`polls_responses.mirrored` (migration `20260818090000_add_mirrored_response.sql`).
The picks alone cannot say, because a mirrored answer looks exactly like a wrong
one, and the two readers of session answers need opposite things from that fact:

- **The community split excludes mirrored rows.** It sells what the room thinks
  the answer *is*, and a mirrored voter was asked for the incorrect options, so
  their picks would invert the signal being priced. A smaller honest sample beats
  a larger misleading one.
- **The run's community board counts them**, graded against the mirrored
...
- The clock is measured client-side (`usePollClock` owns both the countdown and
  the submitted `elapsedMs`, so the display can never disagree with what the
...
- `CorrectnessCheck` gained a third parameter so the standouts module can grade a
  mirrored answer without learning what a poll record looks like.
